...
function setCustomerId(id: string, authState: number) {
  const orgId = process.env.NEXT_PUBLIC_ECID_ORG_ID || "";
  if (!orgId || !window.Visitor) return false;
  try {
...
function fireTriggerView(forceFresh = false) {
  if (!window.adobe?.target?.triggerView) return;
  try {
    // page:true forces a new delivery request instead of serving cached prefetch
    window.adobe.target.triggerView(window.location.pathname, { page: forceFresh });
  } catch {}
}
...
export default function ReaderPicker() {
  ...
  useEffect(() => {
    ...
    setEnabled(
      process.env.NEXT_PUBLIC_DEMO_MODE === "true" || params.get("demo") === "true"
    );
